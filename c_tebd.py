...
def run_TEBD2(psi, U_bonds, half_bonds, N_steps, chi_max, eps):
    Nbonds = psi.L - 1
    assert len(U_bonds) == Nbonds
    for i_bond in range(0, Nbonds,2):
        update_bond(psi, i_bond, half_bonds[i_bond], chi_max, eps)
    for n in range(1,N_steps-1):
        for k in [1,0]:  # even, odd
            for i_bond in range(k, Nbonds, 2):
                update_bond(psi, i_bond, U_bonds[i_bond], chi_max, eps)
...
def example_TEBD_gs_finite_2(L, J, g, h):
    print("finite TEBD, (imaginary time evolution)")
    print("L={L:d}, J={J:.1f}, g={g:.2f}, h={h:.2f}".format(L=L, J=J, g=g, h=h))
    import a_mps
    import b2_model
    model = b2_model.TFIM2(L, J=J, g=g, h=h)
    psi = a_mps.init_spinup_MPS(L)
    for dt in [0.1, 0.01, 0.001, 1.e-4, 1.e-5]:
        half_U_bonds = calc_U_bonds(model, dt/2)
        U_bonds = calc_U_bonds(model, dt)
        run_TEBD(psi, U_bonds, N_steps=500, chi_max=100, eps=1.e-10)
        E = model.energy(psi)
        print("dt = {dt:.5f}: E = {E:.13f}".format(dt=dt, E=E))
...
if __name__ == "__main__":
    _, _, _, U_bonds=example_TEBD_gs_finite(L=14, J=1., g=0.1)
